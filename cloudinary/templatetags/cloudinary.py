...
@register.simple_tag(name='cloudinary_vid', takes_context=True)
def cloudinary_tag(context, vid, options_dict=None, **options):
    if options_dict is None:
        options = dict(**options)
    else:
        options = dict(options_dict, **options)
    try:
        ...
        pass
    if not isinstance(vid, CloudinaryResource):
        vid = CloudinaryResource(vid)
    return mark_safe(vid.video(**options))
